Riot.run(function() {
  context('Ico.SparkLine', function() {
    given('an inline sparkline', function() {
      ...
      should('have a sparkline', sparkline); 
    });

    given('a sparkbar', function() {
      var element   = document.getElementById('sparkline_2'),
          sparkbar  = new Ico.SparkBar(element,
                                       [1, 5, 10, 15, 20, 15, 10, 15, 30, 15, 10],
                                       { width: 30, height: 14, background_colour: '#ccc' });
      should('have a sparkbar', sparkbar); 
    });

    given('a sparkline that needs highlights', function() {
